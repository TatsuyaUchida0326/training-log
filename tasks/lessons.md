...
### 目標
種目詳細モーダルのℹボタン押下後に「読み込み中」のまま止まるバグを修正する。

### 原因特定
`curl` で検証したところ、wger API の `/api/v2/exercise/search/` エンドポイントが 404 を返すことが判明（廃止済み）。そのため `fetch` が完了せず Promise が never settle 状態になり、status が 'loading' から変わらなかった。
...
### 1. 根本原因調査
- wger `/api/v2/exercise/search/` → 404（廃止）
- wger `/api/v2/exerciseinfo/{id}/` → タイムアウト
- wger `/api/v2/muscle/` → 動作するが単体では種目→筋肉マッピング不可

### 2. 代替アーキテクチャへ切り替え
- **筋肉情報**: `exerciseMuscleMap.ts`（39種目のハードコード静的データ）に切り替え
- **説明文**: 日本語 Wikipedia REST API（`ja.wikipedia.org/api/rest_v1/page/summary/{name}`）で直接日本語テキストを取得
- **MyMemory API**: 不要になり除去（翻訳レイヤーなしで日本語テキスト取得）

### 3. ExerciseDetailModal 空説明文の UX 修正
- `descriptionJa` が空文字の場合（Wikipedia ページなし）は説明セクション自体を非表示にする
...
### テスト結果
- 246テスト全件パス（26ファイル）
- useWgerExercise: 8テスト（新アーキテクチャ対応済み）

### よかった点
- `curl` で直接 API エンドポイントを検証し、廃止を即座に特定できた
- Wikipedia API は日本語テキストを直接返すため翻訳不要・高速・安定
- API 呼び出しを 7回 → 1回に削減（大幅な応答速度改善）
- 筋肉データを静的化したことで筋肉情報は常に正確・即座に表示される

### 反省点
- **外部API依存のリスクを事前検証していなかった**: wger の検索エンドポイントが廃止されていることに実装時点で気づけなかった。外部APIを採用する前に実際のエンドポイントを curl で疎通確認すべきだった
- **テスト環境と本番環境の乖離**: モックベースのテストが全件パスしていても、実際の API が壊れていることは検知できない。外部APIを使う場合は統合テスト（実際にAPIを叩く）も検討する
