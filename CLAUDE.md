...
- 仕様書・タスクを渡されたら、途中で確認を挟まず最後まで一気に実行する
- テストでエラーが出た場合は自分で修正・再テストを繰り返し、全件パスするまで自律的に完遂する
- **コミット・プッシュは行わない。** 実装・テスト完了後に止まり、ユーザーの動作確認を待つ
- ユーザーから「コミットして」「プッシュして」の指示を受けて初めて git 操作を行う
...
## Scope
- Working directory: /Users/TatsuyaUchida/proaka_react_app
